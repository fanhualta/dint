...
//    uint64_t max_n = 1000;
    ...
    outFile << "size,tgc encode time,tgc encode size,tgc decode time,ef encode time,ef encode size,ef decode time,dint encode time,dint dictionary size,dint encode size,dint decode time" << std::endl;
...
inline void multi_posting_test()
{
    ds2i::global_parameters params;
    using ds2i::indexed_sequence;
    using ds2i::strict_sequence;
    ...
    outFile.open("result.csv", std::ios::out);
    outFile << "size,tgc encode time,tgc encode size,tgc decode time,ef encode time,ef encode size,ef decode time,dint encode time,dint dictionary size,dint encode size,dint decode time" << std::endl;
    for (uint64_t n = 1000000; n <= 1000000; )
    {
        std::cout << n << std::endl;
...
{
    single_posting_test(100000, 100000);
//    multi_posting_test();
}
